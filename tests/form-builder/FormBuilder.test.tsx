...
function renderForm(onSubmit: (v: unknown) => void) {
    return render(<FormBuilder schema={schema} onSubmit={onSubmit} title="Test Form" />);
}
...
test("отрисовывает обязательные поля объекта", () => {
    ...
    expect(screen.getByRole("heading", { name: /Phones \*/i })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: /Phones 1/i })).toBeInTheDocument();
});

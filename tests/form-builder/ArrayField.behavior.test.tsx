...
test("кнопка удаления недоступна при количестве элементов, равном minItems", async () => {
    ...
    const phonesSection = screen.getByRole("heading", { name: /Phones \*/i }).closest("div")!;
    const addButton = within(phonesSection).getByRole("button", { name: /^Add$/i });

    const card1 = screen.getByRole("heading", { name: /Phones 1/i }).closest("div")!;
